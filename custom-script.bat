@echo off
:: Enhanced RDP Configuration with Drive Redirection - Fixed Version
...
:: Delete partition & extend C drive (with better error handling)
echo [1/6] Configuring disk partitions...
echo     WARNING: This will modify disk partitions!
choice /C YN /M "Continue with disk partition modification"
if errorlevel 2 goto skip_partition
...
:skip_partition
echo     Disk partition skipped by user.
...
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp" /v "fDisableCam" /t REG_DWORD /d 0 /f
if %errorlevel% equ 0 echo     Camera redirection enabled

:: Enable LPT port redirection
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp" /v "fDisableLPT" /t REG_DWORD /d 0 /f
if %errorlevel% equ 0 echo     LPT port redirection enabled

:: Enable COM port redirection
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp" /v "fDisableCcm" /t REG_DWORD /d 0 /f
if %errorlevel% equ 0 echo     COM port redirection enabled

:: Enable clipboard redirection
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp" /v "fDisableClip" /t REG_DWORD /d 0 /f
if %errorlevel% equ 0 echo     Clipboard redirection enabled

:: Enable printer redirection (fixed duplicate key)
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp" /v "fDisableCpm" /t REG_DWORD /d 0 /f
...
reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services" /v "fDisableCam" /t REG_DWORD /d 0 /f
...
sc query "TermService" | find "RUNNING" >nul || sc start "TermService"
sc query "SessionEnv" | find "RUNNING" >nul || sc start "SessionEnv" 
sc query "UmRdpService" | find "RUNNING" >nul || sc start "UmRdpService"
...
netsh advfirewall firewall add rule name="RDP Server Access" dir=in action=allow protocol=TCP localport=3389
netsh advfirewall firewall add rule name="RDP Server Access" dir=out action=allow protocol=TCP localport=3389

:: Optional: Add gaming ports (comment out if not needed)
:: netsh advfirewall firewall add rule name="Server Game TCP" dir=in action=allow protocol=TCP localport=1024-65535
:: netsh advfirewall firewall add rule name="Server Game UDP" dir=in action=allow protocol=UDP localport=1024-65535

:: Disable Windows Defender real-time protection (optional)
choice /C YN /M "Disable Windows Defender real-time protection"
...
echo Current RDP session will be refreshed...
choice /C YN /M "Refresh RDP session now (recommended)"
if errorlevel 1 if not errorlevel 2 (
    echo Logging off current session in 5 seconds...
    timeout /t 5 /nobreak
    logoff
) else (
    echo Please logout and login again manually to apply all changes.
)
